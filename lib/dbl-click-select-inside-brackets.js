  ...
  dblClickSelectInsideBrackets() {
    ...
    atom.workspace.observeTextEditors(function(editor) {
      atom.views.getView(editor).addEventListener('dblclick', function(e) {
        ...
        start_char_idx = editor.getBuffer().characterIndexForPosition(dbl_click_buffer_pos);
        if (forward_search) {
          editor.setCursorBufferPosition(dbl_click_buffer_pos);
          end_pos = editor.getBuffer().positionForCharacterIndex(start_char_idx + i);
          editor.selectToBufferPosition(end_pos);
        }
        else {
